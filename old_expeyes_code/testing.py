...
class AppWindow(QtGui.QMainWindow, testing.Ui_MainWindow):
	...
	def __init__(self, parent=None,**kwargs):
		...
		self.group1size = 4
		self.group2size = self.group1size+4
		self.tests = [
		#group 1 begins
		['CAP330',330e-12,self.CAP],
		['W1-A1',1e3,self.W1A1],
		#['PV1-A1','graph',self.PV1A1],
		#['PV2-A2','graph',self.PV2A2],
		['SQR1-IN2',1e3,self.SQR1IN2],
		['SEN',5.564e3,self.SEN],
		#group 2 begins
		['SQR2-IN2',1e3,self.SQR2IN2],
	...
	def correct(self):
		...
		self.scalers[5] *= self.PCS_SCALING  #slope
		self.scalers[6] *= self.RESISTANCE_SCALING
		self.scalers[7] *= self.CRRC
		
		QtGui.QMessageBox.about(self,'info','loading %s\nPCS SCALING:%s\nCR0 : %.3f\nCR1 : %.3f\nCR2 : %.3f\nCR3 : %.3f\nRES : %.3f\nCap RC : %.3f\n'%(self.scalers,self.PCS_SCALING,self.CR0,self.CR1,self.CR2,self.CR3,self.RESISTANCE_SCALING,self.CRRC))

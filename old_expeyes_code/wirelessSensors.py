...
import time,random,functools,sys,inspect
import numpy as np
...
class AppWindow(QtGui.QMainWindow, wirelessTemplate.Ui_MainWindow,utilitiesClass):
	...
	def newRadioLink(self,**args):
		...
		**Arguments**  Description   
		...
		'''
		from expeyes.radio import RadioLink
		return RadioLink(self.I.NRF,**args)
	...
	class nrfHandler(QtGui.QWidget,wirelessWidget.Ui_Form):
		...
		def getFreq(self):
			self.freqBtn.setText('FREQ(CS3): '+self.applySIPrefix(self.node.readFrequency(),'Hz'))
		def getADC(self):
			self.adcBtn.setText('ADC(CS3): '+self.applySIPrefix(self.node.readADC('CS3'),'V'))
	...
	def updateLogWindow(self):
		...
		self.logs.setHtml(T)
		"""
		lst = self.I.NRF.get_nodelist(True)
		x=self.I.readLog()
		print (x,lst)
		...
		self.nodeWidgets=[]
		for a in lst:
			new = self.newRadioLink(address=a)

...
class AppWindow(QtGui.QWidget, plotTemplate.Ui_Form,expeyesWidgets):
	...
	def __init__(self, parent=None,**kwargs):
		...
		self.TITLE(_translate("diode-iv",'Parameters'))
		self.startVoltage=self.SPINBOX(decimals=True,prefix = _translate("diode-iv",'Start Voltage: '),range=[0,4.9],value = 0)
		self.stopVoltage=self.SPINBOX(decimals=True,prefix = _translate("diode-iv",'Stop Voltage: '),range=[0,4.9],value = 4)
		self.stepVoltage=self.SPINBOX(prefix = _translate("diode-iv",'Steps: '),range=[5,1000],value = 100)

		self.SPACER(10)
		self.minimumTime=self.SPINBOX(prefix = _translate("diode-iv",'acquisition time: '),suffix=_translate("diode-iv",' S'),range=[0,2000],value = 5,tooltip="minimum acquisition time. 0 implies fastest possible")

		#Add a vertical spacer in the widgetLayout . about 0.5cm
		self.SPACER(20)
		self.TITLE(_translate("diode-iv",'Initialize'))
		self.PUSHBUTTON(_translate("diode-iv",'Start Logging') , self.start)
		self.PUSHBUTTON(_translate("diode-iv",'Stop Logging') , self.stop)
		self.SPACER(20)
		self.PUSHBUTTON(_translate("diode-iv",'Fit Data') , self.fit_curve)
		...
		self.plot = self.newPlot([],detailedWidget=True,xMin=0,xMax = 4, bottomLabel = _translate("diode-iv",'voltage'),bottomUnits='V',leftLabel = 'current',leftUnits='A',enableMenu=False,legend=True,autoRange='y')
	...
	def fit_curve(self):
		msg = _translate("diode-iv",'fit failed. please acquire some data first')
		if self.xdata is not None and self.activeCurve is not None:
			...
			if f != None:
				...
				msg = _translate("diode-iv",'Fitted with Diode Equation :\nIo = %s\nIdeality factor = %5.2f')%(self.applySIPrefix(Io,'A',2),n)
				fitcurve = self.addCurve(self.plot,_translate("diode-iv",'%s\n%s\nIF=%5.1f')%(self.activeCurve.name(),self.applySIPrefix(Io,'A',2),n),'#fff')
				fitcurve.setData(self.xdata,f[0])
			else:
				msg = _translate("diode-iv",'Failed to fit the curve ')
		QtGui.QMessageBox.information(self, _translate("diode-iv",'Fit Results'), msg)

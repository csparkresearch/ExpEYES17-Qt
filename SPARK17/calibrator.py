...
import templates.ui_calibrator as calibrator
...
'image' : 'calib.png',
'name':'Device\nCalibrator',
'hint':'''Calibrate your device using a plugged in ADS1115 16-bit ADC and save values to a directory. Use the calibration loader utility to process the results and write to flash. Not for regular users!
'''
}
...
class acquirer():
	...
	def getAnotherPoint(self):
		...
		#step 3 : readd all ADC inputs : A1,2 (all gain settings) ,A3 , and save them
		for a in self.INPUTS:
			if self.I.analogInputSources[a].gainEnabled:
				...
				if self.I.analogInputSources[a].__conservativeInRange__(ADC_ACTUAL):
					...
					print ('out',ADC_ACTUAL,self.ADC_DIRECT2[-1])
...
class AppWindow(QtGui.QMainWindow, calibrator.Ui_MainWindow):
	...
	def __init__(self, parent=None,**kwargs):
		...
		self.I.DAC.setVoltage(self.DAC_CHAN2,0) #0+
		...
		item = self.addLabel('DAC',QtGui.QColor(255,255,255)); self.curves['DAC'].setClickable(True);	self.curves['DAC'].sigClicked.connect(functools.partial(self.selectItem,item))

		self.curves['DAC2']=pg.PlotCurveItem(pen=pg.mkPen([255,0,255], width=2),name='DAC2')
		self.plot.addItem(self.curves['DAC2'])

		item = self.addLabel('DAC2',QtGui.QColor(255,255,255)); self.curves['DAC2'].setClickable(True);	self.curves['DAC2'].sigClicked.connect(functools.partial(self.selectItem,item))
	...
	def upload(self):
		final_fitstr=self.stoa('ExpEYES SLOPES and OFFSETS``%s\n'%(time.ctime()))
...
if __name__ == "__main__":
    ...
    myapp = AppWindow(I=eyes17.open(verbose = True))
    myapp.show()
    sys.exit(app.exec_())

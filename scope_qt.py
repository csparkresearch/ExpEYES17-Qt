...
class AppWindow(QtGui.QMainWindow, layout.Ui_MainWindow,expeyesWidgets):
	...
	def __init__(self, parent=None,**kwargs):
		...
		self.experimentBox.addItem('Select Experiment')
		self.experimentBox.addItems(self.expts.keys())
		self.expt=None

	def launchExperiment(self):
		name = str(self.experimentBox.currentText())
	...
	def displayFit(self, channel, x=[], y=[]):
			...
			cb={'A1':self.FITA1, 'A2':self.FITA2, 'A3':self.FITA3, 'MIC':self.FITMIC}[channel]
			if not len(x):
					cb.setText('')
					return
			# make the fit

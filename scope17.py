...
def update():
	...
	try:
		...
		if Res.get():p.get_average_voltage('A1')
		#p.get_capacitance()
...
Res = IntVar()
Checkbutton(f,text = 'RES', variable = Res).pack(side=LEFT)
...
schoolExpts = [ 
[_('Measuring Voltage'), 'measure-voltage'],
]
...
menubar.add_cascade(label="Device", menu=reconnMenu)
...
for ex in opAmpExpts:
	opAmpMenu.add_command(label=ex[0], command = lambda expt=ex[1] :g.pop_help(expt))
...
for ex in schoolExpts:
	schoolMenu.add_command(label=ex[0], command = lambda expt=ex[1] :run_expt(expt))
